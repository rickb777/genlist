...
func TestTakeOther(t *testing.T) {
	things := OtherSlice{1, 3, 17, 5, 6, 17, 8, 9, 13}
	...
	if len(where2) != 0 {
		t.Errorf("Take should result in empty slice, got %v", where2)
	}
	...
	where4 := OtherSlice{}.Take(100)

	if len(where4) != 0 {
		t.Errorf("Take should result in empty slice, got %v", where4)
	}
}
